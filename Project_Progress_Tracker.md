...
+------------------------------------------------------------------------------+
| [~] PHASE 1: MVP - 基础设施与最小闭环                                        |
+------------------------------------------------------------------------------+
| 目标: 用极少数据跑通完整流程，验证开发环境                                   |
| 输入: 3例正常肺 + 1例COPD                                                    |
|                                                                              |
| 任务流程:                                                                    |
|   [x] 环境配置 -> [ ] 数据清洗 -> [ ] 简单配准 -> [ ] 可视化验证             |
|       Python3.9+     run_segment    选1例正常肺    static_render.py          |
|       PyTorch        clean_bg.py    作临时底座                               |
|       ANTsPy                        register.py                              |
|                                                                              |
| 输出: 3D渲染截图                                                             |
| 验收: [ ] 灰色肺 + 红色病灶清晰可见                                          |
...
+-------+--------+-------+--------+-------+--------+-------+--------+-------+--------+
        ...
        +----------------+----------------+----------------+----------------+
                    ...
                    |            src/utils/ 工具层               |
                    |---------------------------------------------|
                    | io.py | logger.py | metrics.py | data_quality.py | visualization.py |
                    | NIfTI | 统一日志  | SSIM/Dice  | 质量检查        | PyVista封装      |
                    +---------------------------------------------+
                                          |
                                          v
[数据层 data/] ================================================================
