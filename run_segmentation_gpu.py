...
日期: 2025-12-09
更新: 2025-12-14 - 重构为入口脚本，核心逻辑迁移到 src/01_preprocessing/run_segmentation.py
"""
...
# 导入核心模块（使用 importlib 处理以数字开头的模块名）
import importlib
run_segmentation_module = importlib.import_module("src.01_preprocessing.run_segmentation")

check_gpu_available = run_segmentation_module.check_gpu_available
check_totalsegmentator_available = run_segmentation_module.check_totalsegmentator_available
get_default_method = run_segmentation_module.get_default_method
get_default_device = run_segmentation_module.get_default_device
run_totalsegmentator_batch = run_segmentation_module.run_totalsegmentator_batch
run_threshold_batch = run_segmentation_module.run_threshold_batch


def print_environment_check(method: str, device: str) -> bool:
    """
    打印环境检查结果
    ...
    if gpu_ok:
        ...
        if device.startswith("cuda"):
            errors.append(f"❌ GPU 不可用: {gpu_msg}")
        else:
            warnings.append(f"⚠️ GPU 不可用: {gpu_msg} (使用 CPU 模式)")
...
示例：
  ...
  python run_segmentation_gpu.py --type normal --test-run 2
    ...
    # 环境检查
    if not print_environment_check(args.method, args.device):
        sys.exit(1)
    ...
    for data_name, input_dir, mask_dir, clean_dir in data_types:
        ...
        if args.method == 'totalsegmentator':
            results = run_totalsegmentator_batch(
                ...
                device=args.device,  # 直接传递 device 参数（如 cuda:2）
                fast=args.fast,
