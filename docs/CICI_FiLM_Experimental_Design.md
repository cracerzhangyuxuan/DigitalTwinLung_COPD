...
与之对应的是 Multi-level FiLM，后者在解码器每一层都注入调制。

**与 Multi-level FiLM 的对比**：

| 维度 | Multi-level FiLM | Output-level FiLM（本方案） |
|:-----|:-----------------|:--------------------------|
| 注入位置 | 解码器 4 层（每层通道数 256/128/64/32） | 最终输出层（通道数 = 1） |
| 需要访问内部特征 | ✅ 必须修改 forward 或使用 Hook | ❌ 只需要最终输出，纯 Wrapper |
| 可训练 γ/β 数量 | 4 层 × 2 = 8 个投影头，约 12,000 参数 | 1 层 × 2 = 2 个投影头，130 参数 |
| 与 backbone 架构耦合 | ✅ 需要知道各层通道数（UNet vs AttUNet 不同） | ❌ 与任何架构完全解耦 |
| 表达力 | 高（可控制中间特征分布） | 中（仅控制最终输出的全局统计量） |
...
对于更复杂的空间差异控制（如让气肿区域内部形成梯度纹理），
Multi-level FiLM 才会体现出优势——但这需要每个体素有独立的条件向量，
超出了本项目的设计目标。
